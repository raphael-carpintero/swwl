...
rgasp <- function(design, response,trend=matrix(1,length(response),1),zero.mean="No",nugget=0,
                  ...
                  initial_values=NA,num_initial_values=2,seed=-1){
  if (zero.mean=="Yes"){
 ...
 if(!model@isotropic){
  ...
  for(i_p in 1:  model@p){
    ...
    #else if (model@kernel_type[i_p]=="constant"){   ## TODO RCP added this
    #  kernel_type_num[i_p]=as.integer(6)
    #}
  }
  ...
  ##check for R0 # TODO: RCP removed this
  ...
  #}
  print(paste("Size matrices", length(model@R0), model@p))
  cat(paste("Size matrices", length(model@R0), model@p))
  if(length(model@R0)!=model@p){ # TODO RCP added this
    cat("Using some constant kernels") 
  }
    ...
    if(optimization=='lbfgs'){
      for(i_ini in 1:num_initial_values){
        ...
        if(method=='post_mode'){
          ...
          tt_all <- try(nloptr::lbfgs(ini_value, neg_log_marginal_lik, 
                                      neg_log_marginal_lik_deriv,nugget=nugget, nugget.est=model@nugget.est, 
                                      R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
                                      kernel_type=kernel_type_num,alpha=model@alpha,lower=model@LB,
                                      nl.info = FALSE, control = list(maxeval=max_eval)),TRUE)
      ...
      for(i_ini in 1:num_initial_values){
        ...
        if(method=='post_mode'){
          if(prior_choice=='ref_approx'){####this one can be with nugget or without the nugget
            ...
            tt_all <- try(optim(ini_value, neg_log_marginal_post_approx_ref,nugget=nugget, nugget.est=model@nugget.est, 
                                R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
            ...
            tt_all <- try(optim(ini_value, neg_log_marginal_post_ref, 
                                nugget=nugget, nugget.est=nugget.est, R0=model@R0,
                                X=model@X, zero_mean=model@zero_mean,output=model@output, prior_choice=prior_choice, kernel_type=kernel_type_num,
          ...
          tt_all <- try(optim(ini_value, neg_log_profile_lik,nugget=nugget, nugget.est=model@nugget.est, 
                              R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
                              kernel_type=kernel_type_num,alpha=model@alpha),TRUE)
          
        }else if(method=='mmle'){
          tt_all <- try(optim(ini_value, neg_log_marginal_lik,nugget=nugget, nugget.est=model@nugget.est, 
                              R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
                              kernel_type=kernel_type_num,alpha=model@alpha),TRUE)
        ...
        #for(i_ini in 1:num_initial_values){
          ...
          if(method=='post_mode'){
            ...
            if(prior_choice=='ref_approx'){####this one can be with nugget or without the nugget
              ...
              tt_all <- try(optimize(neg_log_marginal_post_ref, 
                                     nugget=nugget, nugget.est=nugget.est, R0=model@R0,
                                     X=model@X, zero_mean=model@zero_mean,output=model@output, prior_choice=prior_choice, kernel_type=kernel_type_num,
            ...
            tt_all <- try(optimize(neg_log_profile_lik,nugget=nugget, nugget.est=model@nugget.est, 
                                   R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
                                   kernel_type=kernel_type_num,alpha=model@alpha,lower=LB,upper=UB),TRUE)
            
          }else if(method=='mmle'){
            tt_all <- try(optimize(neg_log_marginal_lik,nugget=nugget, nugget.est=model@nugget.est, 
                                   R0=model@R0,X=model@X, zero_mean=model@zero_mean,output=model@output, CL=model@CL, a=a,b=b,
                                   kernel_type=kernel_type_num,alpha=model@alpha,lower=LB,upper=UB),TRUE)
...
show.rgasp <- function(object) {	
  ...
  cat('Mean parameters: ',object@theta_hat,'\n')
  cat('Variance parameter: ', object@sigma2_hat,'\n')
  cat('Range parameters: ', 1/object@beta_hat,'\n')
  cat('Noise parameter: ', object@sigma2_hat*object@nugget,'\n')
