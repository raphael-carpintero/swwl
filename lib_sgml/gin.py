...
#         # return dropped_outcome
    ...
    def create_batch(self,graph_data):
        graph_data = [process_data_gin(graph_data,False)]
        batch, batch_indice = create_batch(graph_data,nb_graph = graph_data[0].shape[0],minibatch_size=self.batch_size,shuffle=True)
        batch_labels = [batch_labels(x) for x in batch[0]]
